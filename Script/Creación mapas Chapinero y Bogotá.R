...
library(readr)
HOUSEM<- readRDS("C:/Users/valer/Desktop/Andes/Intersemestral/Big Data/ArchivoPS3/Problem-Set-3/Elementos_Guardados/HOUSEM.rds")
class(HOUSEM)
####---
...
leaflet() %>% addTiles() %>% 
  ...
  addCircles(data= HOUSEBOG , col="red", weight=2) %>% # partamentos
  addCircles(data=barbog , col="orange" , weight=2) %>% 
  addCircles(data = parkbog, col= "green", weight = 2)



####Ahora, se realizará el mapa de Medellín
mnzAnt<-readRDS("C:/Users/valer/Desktop/Andes/Intersemestral/Big Data/ArchivoPS3/Problem-Set-3/Elementos_Guardados/Antioquia.rds") #Datos de manzanas Antioquia
sf_use_s2(FALSE)
